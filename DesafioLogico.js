...
function mergeArrays(array1, array2) {
  // Cria um objeto para armazenar os resultados da mesclagem
  const merged = {};

  // Função auxiliar para adicionar ou mesclar objetos no resultado
  function addOrMerge(obj) {
    if (!merged[obj.id]) {
      // Se o objeto ainda não estiver no resultado, adicione-o
      merged[obj.id] = { ...obj };
    } else {
      // Se já existir um objeto com o mesmo id, mescle as propriedades
      merged[obj.id] = { ...merged[obj.id], ...obj };
    }
  }

  // Itera sobre o primeiro array e adiciona/mescla cada objeto
  array1.forEach(addOrMerge);

  // Itera sobre o segundo array e adiciona/mescla cada objeto
  array2.forEach(addOrMerge);

  // Converte o objeto de resultados para um array e retorna
